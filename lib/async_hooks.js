...
const inspectorHook = createHook({
  init(asyncId, type, triggerId, resource) {
    // TODO(bajtos): support recurring tasks like setInterval
    const recurring = false;
    async_wrap.asyncTaskScheduled(type, asyncId, recurring);
  },

  before(asyncId) {
    async_wrap.asyncTaskStarted(asyncId);
  },

  after(asyncId) {
    async_wrap.asyncTaskFinished(asyncId);
  },

  destroy(asyncId) {
    async_wrap.asyncTaskCanceled(asyncId);
  },
});
